<template>
    <div v-if="user.rules.edit_roles || true" class="row wrapper border-bottom white-bg page-heading">
        <div class="col-lg-10">
            <h2>Add role</h2>
            <ol class="breadcrumb">
    ...
    <form v-if="user.rules.edit_roles || true" method="POST" @submit.prevent="save" :class="{ disabled: editDisabled }">
    ...
    export default {
        ...
        methods: {
            async save() {
                ...
                try {
                    const result = await axios.patch('/vue_api/roles/' + this.$route.params.id, data);
            ...
            async getRole() {
                if(!this.user.rules.edit_roles)
                    return;
                try {
                    const result = await axios.get('/vue_api/roles/' + this.$route.params.id);

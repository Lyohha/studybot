        ...
        methods: {
            ...
            async getUser() {
                if(!this.user.rules.edit_users)
                    return;
                try {
                    const result = await axios.get('/vue_api/users/' + this.$route.params.id);
                    
                    console.log(result.data);
                    this.name = result.data.data.name;
                    this.email = result.data.data.email;

                    for(let item in result.data.data.roles) {
                        this.roles.push(result.data.data.roles[item].id.toString());
                    }
            ...
            async save() {
                ...
                try {
                    const result = await axios.patch('/vue_api/users/' + this.$route.params.id, data);
